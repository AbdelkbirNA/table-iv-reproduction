...
| **FDR** | Fraction of faults whose sampled suite's **oracle** detects the fault | Needs the generated tests' assertions, not just their inputs. A mechanism is now **designed** (record entry-point calls under the reference for *triggered*; run the test against the faulty program for *detected*) — no AST oracle extractor required | design only, in `docs/llm_plain_reconstruction.md` | **None built** | ❌ **Blocker** |
