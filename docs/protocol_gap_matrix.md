...
| **Branch coverage** | Per-test branch adequacy on the program under test | Implemented; adequacy = executed lines **∪** branch arcs, so branch subsumes statement as coverage.py's own branch metric does. Branch points from static analysis; sequential and exception arcs excluded from the arc half | coverage.py 7.15.4 | **Exact** | ✅ Ready |
...
| **FTR** | Fraction of faults whose sampled suite contains a triggering test | Trigger semantics implemented and validated; sampler implemented | **Measured** on 30 real GPT-5-mini faults: mutation 0.387, branch 0.349, statement 0.348 (`results/table_iv_humaneval_gpt5mini.json`) | **Mutation within 0.006 of the paper, statement within 0.037, branch off by 0.101** — branch was 0.282 until an arcs-only adequacy bug was fixed (report §9.1); the remaining gap is open | ✅ Measured under A8 |
